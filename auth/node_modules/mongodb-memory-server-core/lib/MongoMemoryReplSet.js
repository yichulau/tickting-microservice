                ...
                try {
                    ...
                    if (typeof this._replSetOpts.auth === 'object') {
                        ...
                        if (!isInMemory) {
                            ...
                            yield this.stop({ doCleanup: false, force: false }); // stop all servers for enabling auth
                            log('_initReplSet: starting all server again with auth');
                            yield this.initAllServers(); // start all servers again with "auth" enabled
                            yield this._waitForPrimary(); // wait for a primary to come around, because otherwise server selection may time out, this may take more than 30s
                            con = yield mongodb_1.MongoClient.connect(this.getUri(), {
                                authSource: 'admin',
                                authMechanism: 'SCRAM-SHA-256',
                                auth: {
                                    username: this._replSetOpts.auth.customRootName,
